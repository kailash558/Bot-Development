...
namespace Microsoft.BotBuilderSamples
{
    public class DispatchBot : ActivityHandler
    {
        ...
        private async Task DispatchToTopIntentAsync(ITurnContext<IMessageActivity> turnContext, string intent, RecognizerResult recognizerResult, CancellationToken cancellationToken)
        {
            switch (intent)
            {
                ...
                case "q_sample-qna":
                    ...
                    break;
                case "chitchat":
                    await ProcessChitChatSampleQnAAsync(turnContext, cancellationToken);
                    break;
                default:
        ...
        private async Task ProcessChitChatSampleQnAAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken)
        {
            _logger.LogInformation("ProcessChitChatSampleQnAAsync");

            var results = await _botServices.SampleQnA1.GetAnswersAsync(turnContext);
            if (results.Any())
            {
